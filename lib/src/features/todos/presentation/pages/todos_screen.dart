      ...
      if (nextDate != null) {
        ...
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                "Rescheduled to ${DateFormat('MMM d').format(nextDate)}",
              ),
              duration: const Duration(seconds: 4),
              action: SnackBarAction(
                label: "UNDO",
                onPressed: () {
                  todo.dueDate = prevDate;
                  todo.save();
                  if (todo.hasReminder && prevDate != null) {
                    NotificationService().scheduleNotification(
                      id: todo.id.hashCode,
                      title: 'Task Due',
                      body: todo.task,
        ...
        _refreshTodos();
        WidgetSyncService.syncTodos();
        return; // EXIT: Bypass old cloning logic
      }
    }
